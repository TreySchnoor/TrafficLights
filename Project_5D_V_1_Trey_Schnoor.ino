/* Project 5D V.1: Traffic Light Controllers with Pedestrian Lights and emergency overide button activated with keyfob remote.
...
Project 5D V.1: Added emergency overide with button activated with keyfob remote
...
int changeDelay = 200;       //Time in between light changes
int pedestrianDelay = 1000;
...
void loop() {
  ...
  if (digitalRead(pedButton) == HIGH) {  //pedestrian button
    ...
    //Adding Emeregency Traffic Light Change for Ambulance
  ...
  if (digitalRead(RF_D0) == HIGH) { //Your manually overiding the switch when you press button A

    //Serial.println("Button A Pressed");
    changeGreenToRed(eastGreen, eastYellow, eastRed); //This changes the traffic light from Green to Red
    changeRedToGreen(westRed, westYellow, westGreen); //This changes the traffic light from Red to Green
  }
  if (digitalRead(RF_D1) == HIGH) { //Your forcing the traffic lights to change in direction when you press button B

    //Serial.println("Button B Pressed");
    changeGreenToRed(westGreen, westYellow, westRed); //This changes the traffic light from Green to Red
    changeRedToGreen(eastRed, eastYellow, eastGreen); //This changes the traffic light from Red to Green
  }
}
